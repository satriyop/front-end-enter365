...
const purchaseReturnId = computed(() => {
...
const { data: existingPR, isLoading: loadingPR } = usePurchaseReturn(prIdRef)
...
const reasonOptions = [
  ...
  { value: 'excess_quantity', label: 'Kelebihan Kuantitas' },
  { value: 'other', label: 'Lainnya' },
]

function createEmptyItem(): PurchaseReturnItemFormData {
  return {
    description: '',
    quantity: 1,
    unit: 'pcs',
...
const {
  ...
  validationSchema: toTypedSchema(purchaseReturnSchema),
  initialValues: {
...
const { fields: itemFields, push: pushItem, remove: removeItem } = useFieldArray<PurchaseReturnItemFormData>('items')

// Populate form when editing
watch(existingPR, (pr) => {
  if (pr) {
    setValues({
      contact_id: Number(pr.contact_id),
      bill_id: pr.bill_id ? Number(pr.bill_id) : undefined,
      warehouse_id: pr.warehouse_id ? Number(pr.warehouse_id) : undefined,
      return_date: pr.return_date?.split('T')[0] || new Date().toISOString().split('T')[0]!,
      reason: pr.reason ?? '',
      tax_rate: pr.tax_rate ?? 11,
      notes: pr.notes ?? '',
      items: pr.items && pr.items.length > 0
        ? pr.items.map(item => ({
            product_id: item.product_id || undefined,
            bill_item_id: item.bill_item_id || undefined,
...
// Calculations
function calculateItemTotal(item: PurchaseReturnItemFormData): number {
...
const onSubmit = handleSubmit(async (formValues) => {
  const itemsPayload: CreatePurchaseReturnItem[] = (formValues.items || [])
  ...
  const payload = {
    ...
    bill_id: formValues.bill_id || undefined,
...
<template>
  <div class="max-w-5xl mx-auto">
    ...
    <div v-if="isEditing && loadingPR" class="text-center py-12">
      <div class="text-muted-foreground">Loading purchase return...</div>
    </div>
    ...
    <form v-else novalidate @submit.prevent="onSubmit" class="space-y-6">
      ...
      <Card>
        <template #header>
          <h2 class="font-medium text-foreground">Return Details</h2>
        </template>
      ...
      <Card>
        <template #header>
          <div class="flex items-center justify-between">
            <h2 class="font-medium text-foreground">Return Items</h2>
          ...
          <div class="flex justify-end">
            <div class="w-80 space-y-2">
              ...
              <div class="flex items-center gap-2">
                <span class="text-muted-foreground text-sm w-20">Tax (%)</span>
                <input
