    ...
    <template v-else-if="workOrder">
      <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p v-if="workOrder.project" class="text-slate-500 dark:text-slate-400">
            Project:
            <RouterLink :to="`/projects/${workOrder.project.id}`" class="text-primary-600 dark:text-primary-400 hover:underline">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
          <Card>
            <template #header>
              <h2 class="font-medium text-slate-900 dark:text-slate-100">Work Order Details</h2>
            </template>
            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <dt class="text-sm text-slate-500 dark:text-slate-400">WO Number</dt>
                <dd class="font-mono text-slate-900 dark:text-slate-100">{{ workOrder.wo_number }}</dd>
              </div>
              <div>
              ...
              <div>
                ...
                <dd class="font-medium text-slate-900 dark:text-slate-100">{{ workOrder.product?.name ?? '-' }}</dd>
              </div>
              <div>
                <dt class="text-sm text-slate-500 dark:text-slate-400">Project</dt>
              ...
              <div>
                ...
                <dd class="text-slate-900 dark:text-slate-100">{{ workOrder.planned_start_date ? formatDate(workOrder.planned_start_date) : '-' }}</dd>
              </div>
              <div>
                <dt class="text-sm text-slate-500 dark:text-slate-400">Planned End</dt>
                <dd class="text-slate-900 dark:text-slate-100">{{ workOrder.planned_end_date ? formatDate(workOrder.planned_end_date) : '-' }}</dd>
              </div>
              <div>
                <dt class="text-sm text-slate-500 dark:text-slate-400">Actual Start</dt>
                <dd class="text-slate-900 dark:text-slate-100">{{ workOrder.actual_start_date ? formatDate(workOrder.actual_start_date) : '-' }}</dd>
              </div>
              <div>
                <dt class="text-sm text-slate-500 dark:text-slate-400">Actual End</dt>
                <dd class="text-slate-900 dark:text-slate-100">{{ workOrder.actual_end_date ? formatDate(workOrder.actual_end_date) : '-' }}</dd>
              </div>
              <div class="col-span-2">
                <dt class="text-sm text-slate-500 dark:text-slate-400">Description</dt>
                <dd class="whitespace-pre-wrap text-slate-900 dark:text-slate-100">{{ workOrder.description || '-' }}</dd>
              </div>
              <div class="col-span-2" v-if="workOrder.notes">
                <dt class="text-sm text-slate-500 dark:text-slate-400">Notes</dt>
                <dd class="whitespace-pre-wrap text-slate-900 dark:text-slate-100">{{ workOrder.notes }}</dd>
              </div>
            </dl>
          </Card>
          ...
          <Card>
            <template #header>
              <h2 class="font-medium text-slate-900 dark:text-slate-100">Production Progress</h2>
            </template>
            <div class="space-y-4">
              <div class="grid grid-cols-3 gap-4 text-center">
                <div class="p-4 bg-slate-50 dark:bg-slate-800 rounded-lg">
                  <div class="text-2xl font-bold text-slate-900 dark:text-slate-100">{{ workOrder.quantity_ordered }}</div>
                  <div class="text-sm text-slate-500 dark:text-slate-400">Ordered</div>
                </div>
                ...
                <div class="p-4 bg-red-50 dark:bg-red-900/30 rounded-lg">
                  <div class="text-2xl font-bold text-red-600 dark:text-red-400">{{ workOrder.quantity_scrapped }}</div>
                  <div class="text-sm text-slate-500 dark:text-slate-400">Scrapped</div>
                </div>
              </div>
              <div>
                <div class="flex justify-between text-sm mb-1">
                  <span class="text-slate-500 dark:text-slate-400">Completion</span>
                  <span class="font-medium text-slate-900 dark:text-slate-100">{{ formatPercent(workOrder.completion_percentage) }}</span>
                </div>
        ...
        <div class="space-y-6">
          ...
          <Card>
            ...
            <dl class="space-y-3">
              <div class="flex justify-between">
                <dt class="text-slate-500 dark:text-slate-400">Material</dt>
                <dd class="text-slate-900 dark:text-slate-100">{{ formatCurrency(Number(workOrder.estimated_material_cost) || 0) }}</dd>
              </div>
              <div class="flex justify-between">
                <dt class="text-slate-500 dark:text-slate-400">Labor</dt>
                <dd class="text-slate-900 dark:text-slate-100">{{ formatCurrency(Number(workOrder.estimated_labor_cost) || 0) }}</dd>
              </div>
              <div class="flex justify-between">
                <dt class="text-slate-500 dark:text-slate-400">Overhead</dt>
                <dd class="text-slate-900 dark:text-slate-100">{{ formatCurrency(Number(workOrder.estimated_overhead_cost) || 0) }}</dd>
              </div>
              <div class="flex justify-between border-t border-slate-200 dark:border-slate-700 pt-3 font-medium">
                <dt class="text-slate-900 dark:text-slate-100">Total Estimated</dt>
                <dd class="text-slate-900 dark:text-slate-100">{{ formatCurrency(Number(workOrder.estimated_total_cost) || 0) }}</dd>
              </div>
            </dl>
          </Card>
          ...
          <Card>
            <template #header>
              <h2 class="font-medium text-slate-900 dark:text-slate-100">Actual Costs</h2>
            </template>
            <dl class="space-y-3">
              <div class="flex justify-between">
                <dt class="text-slate-500 dark:text-slate-400">Material</dt>
                <dd class="text-slate-900 dark:text-slate-100">{{ formatCurrency(Number(workOrder.actual_material_cost) || 0) }}</dd>
              </div>
              <div class="flex justify-between">
                <dt class="text-slate-500 dark:text-slate-400">Labor</dt>
                <dd class="text-slate-900 dark:text-slate-100">{{ formatCurrency(Number(workOrder.actual_labor_cost) || 0) }}</dd>
              </div>
              <div class="flex justify-between">
                <dt class="text-slate-500 dark:text-slate-400">Overhead</dt>
                <dd class="text-slate-900 dark:text-slate-100">{{ formatCurrency(Number(workOrder.actual_overhead_cost) || 0) }}</dd>
              </div>
              <div class="flex justify-between border-t border-slate-200 dark:border-slate-700 pt-3 font-medium">
                <dt class="text-slate-900 dark:text-slate-100">Total Actual</dt>
                <dd class="text-slate-900 dark:text-slate-100">{{ formatCurrency(Number(workOrder.actual_total_cost) || 0) }}</dd>
              </div>
              <div class="flex justify-between text-sm" :class="Number(workOrder.cost_variance) >= 0 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'">
                <dt>Variance</dt>
          ...
          <Card v-if="workOrder.confirmed_at || workOrder.started_at || workOrder.completed_at">
            ...
            <dl class="space-y-2 text-sm">
              <div v-if="workOrder.confirmed_at" class="flex justify-between">
                <dt class="text-slate-500 dark:text-slate-400">Confirmed</dt>
                <dd class="text-slate-900 dark:text-slate-100">{{ formatDate(workOrder.confirmed_at) }}</dd>
              </div>
              <div v-if="workOrder.started_at" class="flex justify-between">
                <dt class="text-slate-500 dark:text-slate-400">Started</dt>
                <dd class="text-slate-900 dark:text-slate-100">{{ formatDate(workOrder.started_at) }}</dd>
              </div>
              <div v-if="workOrder.completed_at" class="flex justify-between">
                <dt class="text-slate-500 dark:text-slate-400">Completed</dt>
                <dd class="text-slate-900 dark:text-slate-100">{{ formatDate(workOrder.completed_at) }}</dd>
              </div>
              <div v-if="workOrder.cancelled_at" class="flex justify-between text-red-600 dark:text-red-400">
                <dt>Cancelled</dt>
