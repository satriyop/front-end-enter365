...
<template>
  <div>
    ...
    <div v-if="error" class="bg-white rounded-xl border border-slate-200 p-8 text-center">
      <p class="text-red-500">Failed to load bills</p>
    </div>
    ...
    <div v-else-if="bills.length === 0" class="bg-white rounded-xl border border-slate-200">
      <EmptyState
      ...
      <table class="w-full">
        ...
        <tbody class="divide-y divide-slate-200">
          <tr v-for="bill in bills" :key="bill.id" class="hover:bg-slate-50">
            <td class="px-6 py-4">
              <RouterLink :to="`/bills/${bill.id}`" class="text-orange-600 hover:text-orange-700 font-medium">
                {{ bill.bill_number }}
              </RouterLink>
            </td>
