...
const {
  ...
  updateFilter,
  goToPage,
} = useResourceList<SalesReturn, SalesReturnFilters>({
  useListHook: useSalesReturns,
  initialFilters: {
    page: 1,
    per_page: 20,
    status: undefined,
...
function handleStatusChange(value: string | number | null) {
  updateFilter('status', value === '' ? undefined : (value as SalesReturnStatus))
}
...
<template>
  <div>
    ...
    <Card v-if="error" class="text-center py-8">
      <p class="text-red-500">Failed to load sales returns</p>
    </Card>
      ...
      >
        ...
        <template #cell-return_number="{ item }">
          <span class="text-primary-600 dark:text-primary-400 font-medium">
            {{ formatReturnNumber(item as SalesReturn) }}
          </span>
        </template>
        ...
        <template #cell-customer="{ item }">
          <span class="font-medium text-slate-900 dark:text-slate-100">
            {{ (item as SalesReturn).contact?.name || '-' }}
          </span>
        </template>

        <!-- Invoice -->
        <template #cell-invoice="{ item }">
          <span v-if="(item as SalesReturn).invoice" class="text-slate-600 dark:text-slate-400">
            {{ (item as SalesReturn).invoice?.invoice_number }}
          </span>
        ...
        <template #cell-status="{ item }">
          <Badge :class="getSalesReturnStatus(item as SalesReturn).color">
            {{ getSalesReturnStatus(item as SalesReturn).label }}
          </Badge>
        </template>
