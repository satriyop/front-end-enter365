...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="flex gap-2">
        <RouterLink to="/inventory/movements">
          <Button variant="secondary">{{ posChrome('Stock Movements', posPack) }}</Button>
        </RouterLink>
        <RouterLink to="/inventory/adjust">
          <Button>{{ posChrome('Adjust Stock', posPack) }}</Button>
        </RouterLink>
      </div>
    </div>
      ...
      <div class="flex flex-wrap gap-4">
        <div class="flex-1 min-w-[200px]">
          <Input
            ...
            :placeholder="posPack ? 'Cari nama atau SKU...' : 'Search by product name, SKU...'"
    ...
    <div v-if="error" class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-8 text-center">
      <p class="text-red-500 dark:text-red-400">{{ posPack ? 'Gagal memuat stok' : 'Failed to load inventory' }}</p>
    </div>
    ...
    <div v-else-if="isEmpty" class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700">
      <EmptyState
        :title="posPack ? 'Belum ada stok' : 'No stock data'"
        :description="posPack ? 'Stok muncul setelah ada mutasi barang' : 'Stock will appear here when products are added to inventory'"
      />
    </div>
      ...
      >
        ...
        <template #cell-level="{ item }">
          <Badge :variant="getStockLevel(item.quantity)">
            {{ posChrome(item.quantity <= 0 ? 'Out' : item.quantity < 10 ? 'Low' : 'OK', posPack) }}
          </Badge>
        </template>
