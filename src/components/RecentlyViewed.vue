...
<template>
  ...
  <div v-if="variant === 'dropdown'" class="w-80">
    <div class="px-4 py-3 border-b border-slate-200">
      <div class="flex items-center justify-between">
        <h3 class="font-medium text-slate-900">Recently Viewed</h3>
        <button
          v-if="items.length > 0"
          type="button"
    ...
    <div v-else class="max-h-80 overflow-y-auto">
      ...
      >
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-xs text-slate-400 mt-0.5">{{ formatTime(item.viewedAt) }}</p>
        </div>
      </RouterLink>

      <div v-if="hasMore" class="px-4 py-2 text-center border-t border-slate-100">
  ...
  <div v-else class="space-y-1">
    ...
    <RouterLink
      ...
      :key="`${item.type}-${item.id}`"
      :to="item.path"
      class="flex items-center gap-2 px-3 py-2 text-sm text-slate-600 hover:bg-slate-100 rounded-md transition-colors"
    >
